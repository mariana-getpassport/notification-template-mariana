...
- `Makefile` -- contains setup and clean steps to get the repo setup for development.  Add any additional steps for setup or cleanup here.
...
The Makefile contains commands for setup, building, docker and test.  Not all the commands are documented here, but the main ones that developers and build tools will use.
...
- `make setup-dev` -- this command setups up the following: (1) basic installs like pyclean and pre-commit; (2) editable installs for the service packages.
